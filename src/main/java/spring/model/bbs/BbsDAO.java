...
public class BbsDAO {
	...
	/*
	 * 부모글인지 확인하는 메서드. 부모글이 있으면 삭제하지 않는다
	 */
...
//	
	/*
	 * 답변번호를 높여줌. 부모글에 먼저 달린 답변이 있다면 ansnum을 증가시켜주고 그 위로 넣어줌.
	 */
...
//    sql.append("    title   = ?,  ");
//    sql.append("    content = ?  ");
//    if(dto.getFilesize()>0) {//파일이 업로드 되면
//    	sql.append(", filename = ?, ");
//    	sql.append("  filesize = ?");
//    }
//    sql.append(" WHERE bbsno  = ?  ");
//
//    try {
//      int i = 0; // setString은 1, 2, 3 ...으로 가야하는데 if를 쓰면 바꿔야 하니까 인덱스로 처리
//      pstmt = con.prepareStatement(sql.toString());
...
//  	public List<BbsDTO> list(Map map) { mybatis로 바꿔줘서 안쓰는부분
...
//    sql.append(" (SELECT NVL(MAX(bbsno), 0)+1 FROM bbs),");
//    sql.append(" ?, ?, ?, ?, sysdate,  ");
//    sql.append(" (SELECT NVL(MAX(grpno), 0)+1 FROM bbs),?,? )"); //서브쿼리
//
//    try {
//      pstmt = con.prepareStatement(sql.toString());
//      pstmt.setString(1, dto.getWname());
